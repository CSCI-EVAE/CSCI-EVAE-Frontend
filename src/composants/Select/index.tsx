...
import { Select as MuiSelect, MenuItem, FormControl, InputLabel, SelectChangeEvent, FormControlProps } from '@mui/material';
...
const SelectComponent: React.FC<Props> = ({ options, label, value, placeholder, multiple, size, maxSelection, onChange, ...rest }) => {
  const handleChange = (event: SelectChangeEvent<string | number | (string | number)[]>) => {
    ...
    if (multiple && maxSelection) {
      ...
      selectedValue = multiple ? event.target.value : event.target.value as string | number;
    }
  ...
  return (
   ...
   {label && <div style={{ display: 'block' }}>{label}</div>}
    <FormControl sx={{ minWidth: size === 'small' ? 120 : size === 'medium' ? 150 : 250, ...rest.sx }}>
      <MuiSelect
        autoWidth
        value={value}
