...
const Sidebar: React.FC = () => {
  ...
  const handleSidebarDisplay = (isVisible: boolean) => {
    ...
    // if (isVisible) {
    //   document.body.classList.add('sidebar-open'); // Ajoutez la classe au corps du document
    // } else {
    //   document.body.classList.remove('sidebar-open'); // Retirez la classe du corps du document
    // }
  };
      ...
      <style>
        {`
          ...
          .sidebar-container {
            ...
            top: 0;
            left: 0;
            z-index: 2000; /* Ajustez la valeur selon vos besoins */
            /* Autres styles */
          }          
  ...
  {/* {dummyData.map((item) => (
    <SidebarLink key={item.id} href={item.path} className="sidebar-link">
...
<ul className="sidebar-list" style={{ justifyContent: 'center', alignItems: 'center' }}>
            {isAuthenticated() && role === ROLE.admin && (
              ADMIN_DASHBOARD.map((element, index)=> (
                <SidebarLink key={index} href={element.link} className="sidebar-link">
                  <SidebarListItem className="sidebar-item">
                    <span className="sidebar-icon">{element.icon}</span>
